// Includes the header file for Decision structure and related functions
#include "decisions.h"

// Includes utility functions for handling JSON objects
...
{
    // Ensures the number of options is at least 1
    if (num_options < 1)
        printf("[ERROR]: Decision.num_options cannot be zero.");

    // Copies the question text to the decision structure
    strncpy(decision->question_text, question_text, TEXT_LEN);
    decision->num_options = num_options;

    // Allocates memory for the options and copy them if there are any
    if (num_options > 0)
    {
...
void decision_destroy(Decision* decision)
{
    ...
    // Resets the number of options and the options pointer
...
bool decision_from_json(Decision* decision, CharacterModelPool* characters_pool, const cJSON* json)
{
    ...
    // Extracts the question text from the JSON object
    const char* question_text = json_object_get_string(json, "question_text", "");
    Option* options = NULL;
    int num_options = 0;

    // Extracts the options from the JSON object if they exist
    ...
    if (jOptions != NULL && cJSON_GetArraySize(jOptions) > 0)
    {
        ...
        // Populates each option from the JSON array
    ...
    // Initializes the Decision structure with the extracted data
    decision_init(decision, question_text, options, num_options);

    // We free the temporary options array if it was allocated
    if (options != NULL)
        free(options);
...
{
    // Creates a JSON object to represent the decision
    cJSON* json = cJSON_CreateObject();
    cJSON_AddStringToObject(json, "question_text", decision->question_text);

    // Creates a JSON array for the options
    cJSON* jOptions = cJSON_CreateArray();
    for (int i = 0; i < decision->num_options; i++)
    {
        // Converts each option to JSON and add it to the array
...
{
    // Allocates memory for the new Decision structure
    Decision* decision = malloc(sizeof(Decision));
    // Initializes the Decision structure with the provided data
    decision_init(decision, question_text, options, num_options);
    return decision;
}
...
{
    // Destroys the Decision structure and free the allocated memory
